...
export function cleanHtml(html: string, maxTokens = 6000): string {
  let cleaned = html;
  ...
  for (const pattern of STRIP_CLASS_PATTERNS) {
    cleaned = cleaned.replace(
      new RegExp(`<[^>]+class="[^"]*${pattern}[^"]*"[\\s\\S]*?<\\/[^>]+>`, "gi"),
      ""
    );
  }
...
export async function extractBonusData(pageText: string): Promise<ExtractionResult> {
